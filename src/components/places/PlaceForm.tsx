  ...
  return (
    ...
    >
      <div>
        ...
        {results.length > 0 && (
          <ul className="mt-1 flex flex-col divide-y divide-slate-100 rounded-lg border border-slate-200 bg-white">
            {results.map((r, i) => (
              <li key={i}>
                <button
                  type="button"
                  onClick={() => pickResult(r)}
                  className="w-full px-3 py-2 text-left text-xs text-slate-600 hover:bg-aegean-50"
                >
                  {r.display_name}
                </button>
              </li>
