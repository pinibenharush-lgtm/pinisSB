-- Lets you attach a photo to each place. Adds a photo_url column to
-- places, and a public Storage bucket the app uploads into using the
-- same anon key as everything else. Safe to run on an existing
-- project: it only adds a column and a new bucket, it does not touch
-- any existing data.
